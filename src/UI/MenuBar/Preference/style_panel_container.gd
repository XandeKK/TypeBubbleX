...
@export var style : Preference.HQStyles
...
func _ready():
	style_label.text = Preference.hq_styles_string[style]
	font_size_input.text = str(Preference.hq_styles[style].font_size)

	default_font_input.text = Preference.hq_styles[style].default_font if Preference.hq_styles[style].default_font else ''
